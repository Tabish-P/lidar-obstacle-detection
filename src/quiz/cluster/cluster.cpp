...
void proximity(int indice, const std::vector<std::vector<float>>& points, std::vector<int>& cluster, KdTree* tree, float distanceTol, std::vector<bool>& processed)
{
	...
	{
		if(!processed[id])
			proximity(id, points, cluster, tree, distanceTol, processed);
	}
}
